...
public class OrderTable extends JTable implements MouseListener {
    ...
    public Component prepareRenderer(TableCellRenderer renderer, int row, int column) {
        ...
        double open = order.getOpenQuantity();
        ...
        if (rejected) r.setBackground(Color.red);
        else if (canceled) r.setBackground(Color.white);
        else if (open == 0 && executed == 0.0) r.setBackground(Color.yellow);
        else if (open > 0) r.setBackground(Color.green);
        else if (open == 0) r.setBackground(Color.white);
